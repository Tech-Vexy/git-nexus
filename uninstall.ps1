...
Write-Host "🗑️  Uninstalling git-nexus..." -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $BINARY_PATH) {
    ...
    Write-Host "⚠️  Binary not found at: $BINARY_PATH" -ForegroundColor Yellow
    ...
    foreach ($location in $otherLocations) {
        if (Test-Path $location) {
            ...
            try {
                ...
                Write-Host "❌ No write permission. Try running as Administrator:" -ForegroundColor Red
    ...
    if (-not $found) {
        Write-Host "❌ Could not find $BINARY_NAME in common locations" -ForegroundColor Red
        exit 1
    }
}
